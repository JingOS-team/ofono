...
struct ril_network_priv {
	...
	int rat;
	enum ril_pref_net_type nr_network_mode;
	...
	const void *data, guint len, struct ril_registration_state *reg)
{
	...
	else if(ril_status == 10) {                           /*已注册状态应该是1而不是10*/
		reg->status = 1;
	}
	else {
...
static enum ofono_radio_access_mode ril_network_rat_to_mode(int rat)
{
	...
	case NETWORK_MODE_NR_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
		return OFONO_RADIO_ACCESS_MODE_NR;
			...
			struct ril_vendor *vendor)
{
	...
	priv->nr_network_mode = config->nr_network_mode;
