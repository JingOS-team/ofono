...
#define RILMODEM_DEFAULT_NR_MODE    NETWORK_MODE_NR_LTE_GSM_WCDMA
#define RILMODEM_DEFAULT_LTE_MODE   PREF_NET_TYPE_LTE_GSM_WCDMA
#define RILMODEM_DEFAULT_UMTS_MODE  PREF_NET_TYPE_GSM_WCDMA
...
#define RILCONF_NR_MODE                     "nrNetworkMode"
			...
			char *name, guint slot_index)
{
	...
	config->nr_network_mode = RILMODEM_DEFAULT_NR_MODE;
							...
							const char *group)
{
	...
	/* nrNetworkMode */
        if (ril_config_get_integer(file, group, RILCONF_NR_MODE, &ival)) {
                DBG("%s: " RILCONF_NR_MODE " %d", group, ival);
                config->nr_network_mode = ival;
        }
